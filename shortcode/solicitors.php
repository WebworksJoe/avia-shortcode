<?php
/**
 * Solicitor Grid - edited from avia Portfolio Grid
 * 
 * Creates a grid of solicitor custom posts
 */
if ( ! defined( 'ABSPATH' ) ) {  exit;  }    // Exit if accessed directly


if ( !class_exists( 'avia_sc_solicitors' ) )
{
	class avia_sc_solicitors extends aviaShortcodeTemplate
	{
			/**
			...
			function shortcode_insert_button()
			{
				...
				$this->config['name']		= __('Solicitor Grid', 'avia_framework' );
				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-portfolio.png";
				$this->config['order']		= 100;
				$this->config['target']		= 'avia-target-insert';
				$this->config['shortcode'] 	= 'av_solicitors';
				$this->config['tooltip'] 	= __('Creates a grid of solicitors from Solicitor cpt', 'avia_framework' );
					...
					*/
                    ...
                    array(
                        ...
                        "subtype" => array(
                            ...
                            __('Use default solicitor layout',  'avia_framework' ) =>'default')),
					...
					array(
                        "name" 	=> __("Solicitor Grid Image Size", 'avia_framework' ),
                        "desc" 	=> __("Set the image size of the Solicitor Grid images", 'avia_framework' ),
                        "id" 	=> "preview_mode",
                        "type" 	=> "select",
                        "std" 	=> "auto",
                        "subtype" => array(__('Set the Solicitor Grid image size automatically based on column or layout width','avia_framework' ) =>'auto',__('Choose the Solicitor Grid image size manually (select thumbnail size)','avia_framework' ) =>'custom')),

                    array(
                        "name" 	=> __("Select custom image size", 'avia_framework' ),
                        "desc" 	=> __("Choose image size for Solicitor Grid Images", 'avia_framework' ) . "<br/><small>" . __("(Note: Images will be scaled to fit for the amount of columns chosen above)", 'avia_framework' )."</small>",
                        "id" 	=> "image_size",
                        "type" 	=> "select",
					...
					array(
							...
							"desc" 	=> __("When clicking on a solicitor item you can choose to open the link to the single entry, open a preview (aka AJAX Portfolio) or show a bigger version of the image in a lightbox overlay", 'avia_framework' ),
							"id" 	=> "linking",
							"type" 	=> "select",
							"std" 	=> "",
							"subtype" => array(
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$params['class'] = "main_color avia-no-border-styling avia-fullwidth-solicitors {$av_display_classes} ".$meta['el_class'];
	...
	class solicitors_avia_post_grid
	{
		...
		public function html()
		{
			...
			if($linking == "ajax")
			{
				...
				$output .= "<div class='solicitors_preview_container portfolio_preview_container {$container_class}' data-portfolio-id='{$container_id}'>
								...
								<div class='solicitors-details-inner portfolio-details-inner'></div>
							</div>";
			}
			...
			foreach ($this->entries->posts as $entry)
			{
				...
				// Added fields
    			$wwPhone = get_field( 'mobile', $the_id );
    			$wwPosition = get_field( 'position', $the_id );
    			$wwEmail = get_field( 'email', $the_id );
                ...
                if($columns == "1" && $one_column_template == 'special')
                {
                    ...
                    // Added content - solicitor position

                    {
                        $markup = avia_markup_helper(array('context' => 'entry_content','echo'=>false, 'id'=>$the_id, 'custom_markup'=>$custom_markup));
                        $output .= '<div class="entry-content-position">';
                        $output .= "<h3 class='solicitor-grid-position' $markup>";
                        $output .= $wwPosition;
                        $output .= '</h3></div>';
                    }
                    // Added content - solicitor phone

                    {
                        $markup = avia_markup_helper(array('context' => 'entry_content','echo'=>false, 'id'=>$the_id, 'custom_markup'=>$custom_markup));
                        $output .= '<div class="entry-content-phone">';
                        $output .= "<h6 class='solicitor-grid-phone' $markup>";
                        $output .= "<a href='tel:" . $wwPhone . "'>" . $wwPhone . "</a>";
                        $output .= '</h6></div>';
                    }
                    // Added content - solicitor position
                    ...
                    {
                        $markup = avia_markup_helper(array('context' => 'entry_content','echo'=>false, 'id'=>$the_id, 'custom_markup'=>$custom_markup));
                        $output .= '<div class="entry-content-email">';
                        $output .= "<h6 class='solicitor-grid-email' $markup>";
                        $output .= "<a href='mailto:" . $wwEmail . "'>" . $wwEmail . "</a>";
                        $output .= '</h6></div>';
                    }
